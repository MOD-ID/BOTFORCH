    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                ...
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link invite dari Channel!"
                )
                self.LOGGER(__name__).warning(
        ...
        if FORCE_SUB_GROUP:
            try:
                ...
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link invite dari GROUP!"
                )
                self.LOGGER(__name__).warning(
        ...
        try:
            ...
            self.LOGGER(__name__).warning(
                f"Pastikan @{self.username} adalah admin di Channel db anda, CHANNEL_ID Saat Ini: {CHANNEL_ID}"
            )
            self.LOGGER(__name__).info(
                "Bot Berhenti."
            )
        ...
        self.LOGGER(__name__).info(
            f"[🔥BERHASIL DIAKTIFKAN!🔥]\n\nBOT Dibuat oleh @{OWNER}\nJika @{OWNER} Membutuhkan Bantuan, Silahkan Tanyakan di Grup https://t.me/gratisbuku & @{self.username} "
        )
